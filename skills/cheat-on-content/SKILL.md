...
> 🖥️ **Minis 环境说明（本地适配）**
>
> - 本 skill 安装在 `/var/minis/skills/cheat-on-content/`。下文所有相对路径（`skills/`、`shared-references/`、`templates/`、`starter-rubrics/`、`adapters/`、`migrations/`）都相对于该目录解析。
> - Minis 没有 Claude Code 的 slash-command harness，也没有 `.claude/settings.json` 钩子。用**自然语言**触发（"初始化" / "打分这篇 <path>" / "启动预测" / "已发布" / "复盘" / "状态" / "找对标" / "升级 rubric"），然后按路由表**直接读取并执行 `skills/cheat-*/SKILL.md`**。不要依赖 `/cheat-*` 命令是否存在。
> - `hooks/prediction-immutability.sh` 等强制层在 Minis 中不会自动触发。**原则 #1（预测不可变）由你自己在执行时严格遵守**：`## 预测` 段一旦写入就不可修改，之后只能追加 `## 复盘`。需要重做时写新的 `_redo.md` 文件并保留原版。
> - 子技能目录里出现的 `.claude/settings.json`、Task tool sub-agent（channel B）等 Claude Code 专有机制，在 Minis 中用**同一会话内的独立读取**替代：即先只读草稿打分、落盘预测，之后才允许读取任何实绩数据。
> - 内容项目建议建在 `/var/minis/workspace/<你的频道名>/` 下，`/cheat-init` 在该目录里生成 `.cheat-state.json`、`rubric_notes.md`、`predictions/` 等。
