...
> 来自 [benchmark.md](benchmark.md) 的对标账号 **杨秉润YANG**（N=5 样本，import on 2026-09-13，抖音户外装备推荐）。
> **仅定性方向，不直接采纳为数值权重**——5-8 样本拟合容易过拟合。
> 等你自己 N≥5 校准样本后正式 bump 时**再决定**是否调权重。
>
> **本段刻意不含任何真实视频名 / 实绩数字**——rubric_notes.md 是 blind 打分 sub-agent 的白名单文件，
> 只能含通用语言。含证据数据的 Memo 写到 [rubric-memo.md](rubric-memo.md)。

- **看起来重要的维度**: `AB`（问题普遍性）· `HP`（钩子 / 需求锚定）· `QL`（可复制内容密度）· `SAT`（教学人格）
- **看起来不显著的维度**: `SR`（趋势共振）· `NA`（递进感）——高低样本在此维度无差异，无法解释表现差异
- **中等偏重要**: `ER`（情感共鸣）——高样本有明确的"省钱爽感"情绪落点，低样本无情绪落点
- **Claude 给的初始建议**: 对标账号是「**大众刚需 + 价格锚点 + 自嘲人格**」驱动型，不是「技术权威」型。
  若将来 bump，候选方向是把 `AB / HP / QL / SAT` 相对 `SR / NA` 上调。
- **一个便于迁移的推论**（对教程形态）：把「价格锚点」翻译成「**时间 / 步骤成本锚点**」
  （3 分钟 / 5 行代码 / 不用装任何东西），把「自嘲人格」用来对冲教程固有的说教感。

**Status: imported, pre-calibration** —— 你自己的数据出来后本段优先级下降（见 benchmark.md「何时淡出」）。
